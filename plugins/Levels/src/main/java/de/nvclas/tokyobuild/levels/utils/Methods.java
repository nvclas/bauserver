...
public class Methods {
	...
	public static void addXp(Player p, int amount) {
		...
		if(currentXp >= nextLevelXp) {
			for(; currentXp >= nextLevelXp; level++) {
				...
				p.sendMessage(AntiBlockUpdate.PREFIX + "�aGl�ckwunsch, du bist nun �bLevel " + Integer.valueOf(level + 1));
	...
	public static void addAchievement(Player p, String achievement, int xp) {
		...
		p.sendMessage(AntiBlockUpdate.PREFIX + "�7�k11111111111111111111111111111111");
		p.sendMessage(AntiBlockUpdate.PREFIX);
		p.sendMessage(AntiBlockUpdate.PREFIX + "�aErfolg freigeschaltet�8: �e" + achievement);
		p.sendMessage(AntiBlockUpdate.PREFIX + "�b+" + xp + "XP");
		p.sendMessage(AntiBlockUpdate.PREFIX);
		p.sendMessage(AntiBlockUpdate.PREFIX + "�7�k11111111111111111111111111111111");
	...
	public static void levelUp(Player p) {
		...
		if(getLevel(p) == 10) {
			...
			p.sendMessage(AntiBlockUpdate.PREFIX + "�aDu hast den Rang �eDeshi �aerreicht!");
		} else if(getLevel(p) == 20) {
			user.addGroup("puro");
			user.removeGroup("deshi");
			p.sendMessage(AntiBlockUpdate.PREFIX + "�aDu hast den Rang �6Puro �aerreicht!");
		} else if(getLevel(p) == 30) {
			user.addGroup("senpai");
			user.removeGroup("puro");
			p.sendMessage(AntiBlockUpdate.PREFIX + "�aDu hast den Rang �9Senpai �aerreicht!");
		} else if(getLevel(p) == 50) {
			user.addGroup("sensei");
			user.removeGroup("senpai");
			p.sendMessage(AntiBlockUpdate.PREFIX + "�aDu hast den Rang �cSensei �aerreicht!");
		}
	}
